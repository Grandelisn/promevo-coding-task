...
const LABEL_LIST_VISIBILITY_VALUES = ['labelShow', 'labelShowIfUnread', 'labelHide'];
...
export const labelSchema = yup.object().shape({
  ...
  messageListVisibility: yup
    .string()
    .oneOf(MESSAGE_LIST_VISIBILITY_VALUES, 'Invalid message list visibility option'),

  labelListVisibility: yup
    .string()
    .oneOf(LABEL_LIST_VISIBILITY_VALUES, 'Invalid label list visibility option'),

  type: yup
    .string()
    .oneOf(TYPE_VALUES, 'Invalid label type')
    .nullable(),
